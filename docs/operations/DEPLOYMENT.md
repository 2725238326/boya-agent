...
面向读者：维护服务器的开发者或运维人员。
文档状态：当前部署方案；更新时间：2026-09-07。
事实范围：部署样例和启动代码已核对；当前生产实例使用 `49.233.248.86`、域名 `buaaboya.top`，并已完成 HTTPS、Nginx、systemd、公开/管理边界和新版本发布验收。2026-09-04 曾观察到真实课程抓取和邮件 outbox 投递；2026-09-07 发布时当前课程周期暂无可选课程；2026-09-08 起邮件自动课程通知和每日摘要已暂停，Telegram 和自动选课保持关闭，长期稳定性继续观察。
...
依赖包上传优化：部署前先计算 `requirements.txt` 与 `constraints.txt` 的内容哈希，并与服务器 `$APP_DIR/.deps-sha256` 中的上一次成功部署标记比对。依赖未变化时跳过 wheelhouse 构建、SCP 上传和离线安装（GitHub 云端到生产机实测带宽约 29KB/s，200MB+ 依赖包上传曾是流水线主要耗时）。标记只在部署验收全部通过后写入；服务器虚拟环境缺失或损坏时仍走完整安装流程。

更新后按 [RUNBOOK.md](RUNBOOK.md) 做健康检查。不要在没有备份和现场确认的情况下删除数据库、上传文件或旧日志。

## 常规发布节奏

生产发布遵循“候选版本验证、变更审阅、受控部署、业务观察”的顺序：

1. 在本地运行 `python scripts/verify_release.py --require-clean`，确认工作区没有凭据、数据库、日志或上传文件，并保存检查结果。
2. 将同一批改动形成可追踪 Git 提交，推送功能分支并等待 CI；与 `main/master` 的差异、数据库影响、外部通道和回退提交完成审阅。
3. 仅在 `main` 或 `master` 的受控工作流中部署生产。功能分支不会通过生产部署工作流；需要其他环境时应使用独立的环境配置。
