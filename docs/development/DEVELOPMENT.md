...
公开首页回归：运行 `uv run --python .venv scripts/verify_home_browser.py`，可加 `--screenshots` 把固定样例截图输出到忽略版本控制的 `logs/`。检查无登录浏览、报名状态筛选、输入转义、键盘详情展开、失败恢复及邮件暂停。CI 和部署前验证均执行该脚本。
...
需要实际改动项目时，先阅读 [项目改进工作提示词](PROJECT_IMPROVEMENT_PROMPT.md) 和 [改进清单](../project/IMPROVEMENT_BACKLOG.md)。每一轮重要改动都先建立当前基线，再按一个可验证的垂直切片实施，不把多个未验证的大重构一次性混在一起。

进度、测试、部署、风险和下一步统一按照 [改进与汇报标准](../project/REPORTING_STANDARD.md) 记录。报告必须区分“已完成”“已验证”“已部署”“待验证”和“未运行”；服务器健康检查不能替代真实业务演练。

用户页面、邮件、文档和汇报中的文字，按照根目录的 [PLAIN_LANGUAGE_REVIEW_PROMPT.md](../../PLAIN_LANGUAGE_REVIEW_PROMPT.md) 审阅。普通用户不直接面对内部状态值，工程文档在必要时保留内部名称并解释其作用。
...
uv run --python .venv python -m playwright install chromium
```
