...
该记录属于历史发布批次；当前本地验证统一使用 `uv run --python .venv scripts/verify_release.py`。历史批次的测试结果和跳过项仍按当时记录保留，不能替代当前收尾验证。
...
这批验证不覆盖真实课程变化、全流程屏幕阅读器操作或长期并发负载。门户主脚本尚未全部纳入 TypeScript 检查。当前已建立与 CI 一致的 uv 锁定依赖入口；性能尚无新的前后对比数据，不宣称提速。
