...
## 2026-09-08：公开课程浏览与服务状态

`990d404` 已部署。首页新增课程浏览、筛选和详情；统一邮件暂停提示，分开页面加载与采集时间，并修复未来开选课程遗漏。线上公开浏览和个人权限检查通过，邮件保持关闭。见 [发布记录](PUBLIC_BROWSING_2026_09_08.md)。

## 2026-09-08：门户恢复与移动端修复

`e72589b` 已部署，包含登录状态失败重试、失效页签恢复、已满课程键盘操作及移动端布局修复。生产部署前已执行八组离线浏览器回归。本地 Python 3.12 隔离环境检查通过；邮件保持关闭。见 [门户发布记录](PORTAL_RELEASE_2026_09_08.md)。

## 2026-09-08：邮件暂停保护与门户回归

应用 `0b44182` 已部署。邮件默认关闭；暂停提示、队列处理、空课程文字与部署并发策略已更新。测试、备份、生产核对及固定样例浏览器检查见 [发布验证记录](RELEASE_VALIDATION_2026_09_08.md)。
