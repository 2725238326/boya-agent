...
本次只更新项目文档和清晰表达规则，不改变应用代码、数据库结构、生产配置或外部通知开关，也不重启生产服务。文档已经与当前生产事实、运行边界和后续观察范围重新对齐。

### 更新范围

- 文档导航补充 `https://buaaboya.top` 当前入口，并说明哪些提交会自动启动生产工作流。
- 架构文档补充等待策略、缓存、查询、通知任务和单实例 SQLite 的性能边界。
- API、配置、开发、安全和部署文档更新最近核对日期，明确当前生产路径和配置来源。
- 用户行为文档补充“当前无课程”“筛选无结果”和“课程数据暂时不可用”的区别及对应操作。
- 项目状态、改进清单、上线前检查项、报告标准和语言审阅记录统一使用具体、可核对的表达；历史实施记录明确标注为历史资料。

### 文档检查

- 非归档 Markdown：28 份；本地链接：72 个，均可解析。
- `D:\\Anaconda\\python.exe scripts/verify_release.py`：`66 passed, 1 skipped`；编译、JavaScript、TypeScript 7 和差异检查通过。
- 本次文档提交同步到 `main` 和服务器仓库后，生产应用仍运行 `main@92e53b2`；文档或工作流变更不会触发应用重启。
...
## 2026-09-08：暂停生产邮件，准备最终新版

### 配置变更

- 生产数据库中 `email_enabled` 和 `daily_summary_enabled` 已从 `1` 改为 `0`；Telegram 和自动选课保持 `0`。
- 修改前已生成备份：`/var/lib/boya-agent/backups/boya_agent-email-paused-20260908-024504.db`。
- 修改后没有待处理邮件通知任务，未发送选课提醒数量为 `0`；服务未重启，`boya-agent` 和 Nginx 保持 active。
- 本次只暂停调度器自动邮件路径，没有调用邮箱验证/登录、管理员测试邮件、手动推送或站点通知接口。

### 当前上线计划

最终新版按 [FINAL_RELEASE_PLAN.md](FINAL_RELEASE_PLAN.md) 执行。邮件总开关、固定依赖入口、页面和异常状态验收、Playwright 故障样例、数据库检查和生产回退记录均属于上线前事项；在这些检查完成前不恢复邮件。
