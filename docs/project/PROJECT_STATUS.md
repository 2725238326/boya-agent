...
**用户决策与调研结论（同日确认）**：
- **自动选课冻结**：目前不搞，相关代码只做清债维护，不安排专项验证、不做新投入。
- **飞书数量/成本**：每群最多 15 个机器人；自定义 webhook 机器人完全免费、不占 OpenAPI 配额（仅频率限制 100 次/分、5 次/秒、20KB/条）；自建应用免费版有月度 API 调用配额（2026 年限时 100 万次/月，付费版不限量）。结论：告警/群推走 webhook 零成本，为下一步接入首选。

**待办**：飞书自定义机器人接入列入 backlog P1（等待用户提供群机器人 webhook）；Telegram 选课结果通知修复属清债，随自动选课冻结不单独验证。

## 2026-09-17 二维码共享页重构：操作顺序、信息去重、贡献面板补全（`972e7d3` 已部署）

**改动内容**：
- 页面结构按真实任务排序：二维码列表前置（主流场景是"来扫码"），上传表单移到其后；课程信息从三处重复收敛为顶部一条紧凑信息条（签到方式徽章 + 课程名 + 时间/地点/校区单行）。
- 补上此前一直缺失的贡献面板：JS 每轮都拉取 `qrcodeCurrentUser`/上传计数/下期门槛/本期与累计榜单，但模板里这些元素不存在，数据白拉。现已渲染为"贡献与奖励"区块，奖励闭环对用户可见。
- 上传区改为拖拽+点选二合一的 dropzone，选图后本地预览缩略图；上传表单内的重复课程摘要框移除。
- 文案与真实流程对齐：原"上传后自动出现在列表"改为"先进入审核，通过后显示"（API 实际要求管理员审核）。
- 已结束课程的页面上传表单直接停用（此前要上传后才被 API 拒绝）。
- 无障碍：状态框加 `role="status" aria-live="polite"`，列表 `aria-busy`；预览弹窗打开时焦点移到关闭按钮、Esc/关闭后焦点还原到触发卡片。
- `_format_course_time` 同日期压缩：`2026-09-23 14:30 ~ 2026-09-23 16:30` → `2026-09-23 14:30 ~ 16:30`。
- 新增 `scripts/verify_qrcode_browser.py` 离线浏览器检查（列表顺序、卡片渲染、弹窗焦点、dropzone 选图预览、上传提交、榜单渲染、双宽度无横向溢出），已接入 CI verify。

**本地验证（已运行）**：`npm run check` 通过；`pytest` 101 项全过；`verify_qrcode_browser.py` 4 场景全过；桌面/移动截图确认布局。

**部署复核（已完成）**：`ba75167` + `972e7d3`（隐藏 required file input 改为 JS 校验，避免不可聚焦控件的静默提交失败）经 CI 全自动部署；生产 `972e7d3`、active、healthz/公开页 200、1 个 driver。
