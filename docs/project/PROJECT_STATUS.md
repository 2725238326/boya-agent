...
判定范围：仓库代码、配置样例、部署样例、测试文件和静态检查结果。
重要限制：本地 `boya_agent.db` 是 0 字节空文件；没有连接生产服务器，因此本文不把线上运行状态当作已验证事实。

## 一句话结论

项目是一个单进程轻量服务：抓取北航博雅课程，写入 SQLite，按统一课程状态和用户偏好推送到邮件/Telegram，并提供公开课程页、订阅页、用户门户、二维码共享页和管理员后台。当前重构重点已经完成认证、管理员边界、二维码公开隐私、时间和课程状态收口；生产上线前仍需完成依赖安装、TLS 实测和完整回归。
...
1. 默认 Python 3.13 环境缺少 SQLAlchemy 和 Playwright 等项目依赖；`python -m pytest -q` 在收集阶段报告 4 个模块缺依赖，完整测试尚未运行。已有依赖较多的 Anaconda 环境可以运行二维码、课程状态、RSS 空数据、自动选课熔断和调度回归测试，但不能替代完整套件；Web 安全测试仍因缺少 `flask_cors` 未收集。
2. 生产证书、Nginx 实际加载结果、systemd 用户权限、SMTP/Telegram 可达性和外部 SSO 行为尚未在服务器上实测。
...
1. 安装锁定版本依赖，在同一环境运行完整 pytest 和启动后接口回归。
2. 用真实 Nginx + TLS + systemd 做一次端到端订阅、登录、门户、二维码和管理边界演练。
3. 为一次性挑战、代码并发消费、RSS/Atom 和推送失败分类补充更多集成测试。
4. 再考虑拆分 `web/app.py`、`src/scheduler.py` 和前端大文件；拆分必须以测试保护为前提。
