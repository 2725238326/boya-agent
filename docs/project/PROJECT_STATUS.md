...
2026-09-08 应用更新至 `990d404`。首页无需登录浏览和筛选课程，个人操作仍受会话保护；页面明确显示邮件暂停及采集时间。生产版本的发布验证已通过；当前工作树补充改动在本地完成 `84 passed, 1 skipped`、前端检查和编译检查。具体范围与剩余限制见 [公开浏览发布记录](PUBLIC_BROWSING_2026_09_08.md)。以下为此前批次记录。
...
7. outbox 已覆盖课程邮件、课程 Telegram、选课提醒（邮件/Telegram 各自独立任务）和 Telegram 每日汇总；站点调整通知和自动选课结果仍保留旧的直投路径，后续继续统一。上述提醒和汇总迁移为 2026-09-09 的本地改动，尚未部署到生产。
