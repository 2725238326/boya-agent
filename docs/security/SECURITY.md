...
面向读者：开发者、运维人员和安全审阅者。
文档状态：当前实现与待确认风险；更新时间：2026-09-07。
相关代码：`web/security.py`、`web/app.py`、`src/models.py`、`web/qrcode_feature.py`。

最近生产核对：2026-09-07 已检查 HTTPS 跳转、健康端点、未认证管理接口、静态资源边界、服务账户和数据库完整性；这些结果只代表本次核对，不替代证书变更或配置变更后的再次检查。
...
6. TLS、证书续期、Cookie 属性和 Nginx 规则在 2026-09-07 的生产核对中通过；每次证书、域名或 Nginx 配置变更后仍需重新用 `nginx -t`、浏览器和 curl 验证。
